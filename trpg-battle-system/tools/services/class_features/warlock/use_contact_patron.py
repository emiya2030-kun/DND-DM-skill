from __future__ import annotations

from typing import Any

from tools.models.encounter import Encounter
from tools.models.encounter_entity import EncounterEntity
from tools.repositories.encounter_repository import EncounterRepository
from tools.services.class_features.shared import ensure_warlock_runtime
from tools.services.encounter.get_encounter_state import GetEncounterState


class UseContactPatron:
    def __init__(self, encounter_repository: EncounterRepository):
        self.encounter_repository = encounter_repository
        self.get_encounter_state = GetEncounterState(encounter_repository)

    def execute(
        self,
        *,
        encounter_id: str,
        actor_id: str,
    ) -> dict[str, Any]:
        encounter = self._get_encounter_or_raise(encounter_id)
        actor = self._get_actor_or_raise(encounter, actor_id)

        warlock = ensure_warlock_runtime(actor)
        contact_patron = warlock.get("contact_patron")
        if not isinstance(contact_patron, dict) or not bool(contact_patron.get("enabled")):
            raise ValueError("contact_patron_not_available")
        if not bool(contact_patron.get("free_cast_available")):
            raise ValueError("contact_patron_unavailable")

        contact_patron["free_cast_available"] = False
        self.encounter_repository.save(encounter)
        return {
            "encounter_id": encounter_id,
            "actor_id": actor_id,
            "class_feature_result": {
                "contact_patron": {
                    "spell_id": str(contact_patron.get("spell_id") or "contact_other_plane"),
                    "cast_without_spell_slot": True,
                    "auto_succeeds_save": bool(contact_patron.get("auto_succeeds_save")),
                }
            },
            "encounter_state": self.get_encounter_state.execute(encounter_id),
        }

    def _get_encounter_or_raise(self, encounter_id: str) -> Encounter:
        encounter = self.encounter_repository.get(encounter_id)
        if encounter is None:
            raise ValueError(f"encounter '{encounter_id}' not found")
        return encounter

    def _get_actor_or_raise(self, encounter: Encounter, actor_id: str) -> EncounterEntity:
        actor = encounter.entities.get(actor_id)
        if actor is None:
            raise ValueError(f"actor '{actor_id}' not found in encounter")
        return actor
