...
class ExecuteSpellTests(unittest.TestCase):
    ...
    def test_execute_fireball_missing_save_rolls_returns_structured_error_without_side_effects(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            tmp_path = Path(tmp_dir)
            encounter_repo = EncounterRepository(tmp_path / "encounters.json")
            event_repo = EventRepository(tmp_path / "events.json")
            ...
            self.assertFalse(result["ok"])
            self.assertEqual(result["error_code"], "missing_save_rolls")
            self.assertCountEqual(
                result.get("missing_target_ids", []),
            ...
            self.assertEqual(updated.entities["ent_fireball_target_failed_001"].hp["current"], 40)
