...
class ExecuteSpell:
        ...
        if prepared_save_damage is not None:
            ...
            for target_id in target_ids:
                ...
                if not isinstance(roll_input, dict):
                    return self._error(
                        "missing_save_rolls",
                        f"缺少目标 {target_id} 的豁免骰结果",
                        missing_target_ids=[target_id],
                    )
                vantage = roll_input.get("vantage", "normal")
                if not isinstance(vantage, str):
                    vantage = "normal"
        ...
        if prepared_attack_spell is not None:
            ...
            for beam in prepared_attack_spell["beams"]:
                roll_result = RollResult(
                    ...
                    final_total=beam["attack_roll"]["final_total"],
                    dice_rolls=beam["attack_roll"]["dice_rolls"],
                    metadata={},
                )
    ...
    def _is_save_damage_area_spell(self, spell_definition: Any) -> bool:
        ...
        if not isinstance(resolution, dict):
            return False
        if resolution.get("mode") != "save_damage":
            return False
        ...
        if not isinstance(on_cast, dict):
            return False
        return isinstance(on_cast.get("on_failed_save"), dict) and isinstance(on_cast.get("on_successful_save"), dict)
    ...
    def _prepare_save_condition_resolution(
        ...
        *,
        ...
        missing_target_ids = [target_id for target_id in normalized_target_ids if target_id not in save_roll_index]
    ...
    def _prepare_attack_spell_resolution(
        ...
        *,
        ...
        for index, target_id in enumerate(normalized_target_ids):
            ...
            if attack_roll_entries[index]["final_total"] >= target.ac and damage_roll_entries[index] is None:
                return self._error(
    ...
    def _prepare_save_damage_resolution(
        ...
        *,
        ...
        missing_target_ids = [target_id for target_id in target_ids if target_id not in save_roll_index]
        if missing_target_ids:
            return self._error(
                "missing_save_rolls",
                "缺少区域目标的豁免骰结果",
